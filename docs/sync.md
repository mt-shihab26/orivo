...
`repo_name` (`src/config/sync.rs`) defaults to `"Todos"` in release builds and
`"Todos-dev"` in debug builds, so a local `cargo run sync` during development can never
...
checking for github repo Todos...
found existing repo <owner>/Todos
fetching latest changes from <owner>/Todos...
...
checking github cli sign-in...
checking for github repo Todos...
creating private repo <owner>/Todos...
cloning <owner>/Todos...
comparing local database with github...
