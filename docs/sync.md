...
`repo_name` (`src/config/sync.rs`) defaults to `"Notes"` in release builds and
`"Notes-dev"` in debug builds, so a local `cargo run sync` during development can never
...
checking for github repo Notes...
found existing repo <owner>/Notes
fetching latest changes from <owner>/Notes...
...
checking github cli sign-in...
checking for github repo Notes...
creating private repo <owner>/Notes...
cloning <owner>/Notes...
comparing local database with github...
